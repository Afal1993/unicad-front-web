...
const List: React.FC = () => {
  ...
  const getAllDeliveries = () => {
    return api.get('http://localhost:3001/v1/locales').then((res: any) => {
  ...
  const deleteDelivery = (id: number, refresh: any) => {
    return api.delete(`http://localhost:3001/v1/locales/${id}`).then((res: any) => {
    }).catch((e: any) => {
      console.log(e.error)
    })
  }
        ...
        <TableBody>
          {locales.map((row: any) => (
            ...
            >
              ...
              <TableCell align="right"><Button onClick={() => deleteDelivery(row.id, location.reload())}>DELETAR</Button></TableCell></TableRow>
          ))}
        </TableBody>
      </Table>
    </TableContainer>
